...
Usage:
    flowbook_split input.ipynb -o output.ipynb
    flowbook_split input.ipynb  # Output: input_split.ipynb
"""
...
from flowbook.cli.helpers import load_notebook, save_notebook
...
async def split_notebook(
    ...
    with timer(key="cli:split_total", message="Split notebook CLI"):
        ...
        if model:
            config = FlowbookConfig(model=model)
        else:
...
def split_cli_main():
    """Entry point for flowbook_split CLI."""
...
Examples:
  ...
  flowbook_split notebook.ipynb -o notebook_split.ipynb

  # Split a notebook with custom model
  flowbook_split notebook.ipynb --model claude-3-5-sonnet-20241022

  # Split a notebook (output defaults to notebook_split.ipynb)
  flowbook_split notebook.ipynb
        """,
    )
    parser.add_argument(
        "input",
