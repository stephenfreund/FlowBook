"""
Optimization pipeline CLI for flowbook notebook processing.
...
from flowbook.util.output import error, indent, log, timer, quiet, print, output
    ...
    with indent(message=f"[{cell_index}/{total_cells}] Processing cell {cell_id}"):
        ...
        with indent(message=f"* Profiling..."):
            ...
            for c in notebook_content.get("cells", []):
                if c.get("id") == cell_id:
                    ...
                    flowbook_metadata = FlowbookMetadata.from_cell(c)
                    profile_data = flowbook_metadata.get_profile()
                    if profile_data:
                ...
                try:
                    ...
                    if cell:
                        ...
                        optimization_potential = (
                            flowbook_metadata.get_optimization_potential()
                        )
        ...
        if should_optimize:
            with indent(message=f"* Optimizing..."):
                ...
                from flowbook.server.commands.optimize import PrePostEnvironments
...
async def run_split_preprocessing(
    notebook_content: Dict[str, Any], config: FlowbookConfig, registry: CommandRegistry
) -> Dict[str, Any]:
    """
...
def optimize_cli_main():
    ...
    parser.add_argument(
        ...
        help="Kernel name for new kernel (default: flowbook_kernel)",
    )
    ...
    with quiet_context:
        ...
        try:
            ...
            for cell_result in metadata["cell_results"]:
                ...
                # Find the actual cell in the notebook to get its flowbook metadata
                cell = None
                for c in notebook_content["cells"]:
                    if c.get("id") == cell_id:
                        cell = c
                ...
                # Get profile duration and optimization potential from cell's flowbook metadata
            ...
            result["notebook"]["metadata"][
                "flowbook_optimization"
            ] = flowbook_metadata.model_dump()
