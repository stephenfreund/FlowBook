...
Usage:
    python -m flowbook.testing.scripts.run_performance notebook.ipynb [options]
...
def main():
    ...
    with timer(key="script:run_tests", message=f"Running {total_iterations} performance tests"):
    ...
    print(f"{'Cell ID':<10} {'Count':<7} {'Clean (ms)':<12} {'Modified (ms)':<14} {'Vars Checked':<12}")
    print(f"{'-'*10} {'-'*7} {'-'*12} {'-'*14} {'-'*12}")
    for cell_id, stats in cell_stats.items():
        # Each experiment runs both clean and modified, so count = number of clean runs
        count = len(stats["clean"])
        ...
        print(f"{cell_id:<10} {count:<7} {clean_avg:<12.3f} {mod_avg:<14.3f} {vars_avg:<12.1f}")
