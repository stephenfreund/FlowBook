...
from flowbook.util.flowbook_metadata import (
    ...
    UnitTest,
    UnitTests,
    set_unit_tests_flowbook_metadata,
)
...
class GenerateTestsCommand(NotebookCommand):
    ...
    async def _generate_tests_for_cell(
        ...
        model: str,
        context: str = "",
    ) -> Tuple[List[UnitTest], FlowbookStats]:
...
"""
        ...
        env_data = flowbook_meta.profile.env if flowbook_meta.profile else None
        ...
        generated_tests, stats = await FlowbookAgent.make_and_run_agent(
            key="generate-tests",
            model=model,
    ...
    async def generate_tests_for_cells(
        ...
        for cell_id, result in results:
            ...
            if result.tests:
                ...
                flowbook_meta = FlowbookMetadata.from_cell(cell)
                existing_tests = (
                    flowbook_meta.unit_tests.tests if flowbook_meta.unit_tests else []
                )
                ...
                set_unit_tests_flowbook_metadata(cell, unit_tests)
