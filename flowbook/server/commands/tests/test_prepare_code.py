"""
Tests for the PrepareCodeForFlowBook command.
"""
...
@pytest.fixture
def command():
    """Create a PrepareCodeForFlowbookCommand instance."""
    return PrepareCodeForFlowbookCommand()
...
def test_simple_column_fillna(command, base_notebook):
    ...
    result = command.process(base_notebook)
...
def test_simple_column_replace(command, base_notebook):
...
def test_boolean_mask(command, base_notebook):
...
def test_nested_subscripts(command, base_notebook):
...
def test_multiple_transformations_per_cell(command, base_notebook):
...
def test_no_transformation_direct_method(command, base_notebook):
...
def test_no_transformation_no_inplace(command, base_notebook):
...
def test_no_transformation_inplace_false(command, base_notebook):
...
def test_preserve_comments(command, base_notebook):
...
def test_preserve_multiline_formatting(command, base_notebook):
...
def test_skip_markdown_cells(command, base_notebook):
...
def test_skip_empty_cells(command, base_notebook):
...
def test_handle_syntax_errors_gracefully(command, base_notebook):
...
def test_selected_cells_only(command, base_notebook):
    ...
    # Only process cell2
    result = command.process(base_notebook, selected_cell_ids=["cell2"])
...
def test_entire_notebook_when_no_selection(command, base_notebook):
...
def test_command_properties(command):
    """Test command properties."""
    assert command.command_name == "prepare_code"
    assert command.display_name == "Prepare Code for FlowBook"
    assert command.icon_name == "ui-components:code"
    assert command.requires_kernel is False


def test_metadata_structure(command, base_notebook):
...
def test_timing_recorded(command, base_notebook):
...
def test_list_source_format(command, base_notebook):
