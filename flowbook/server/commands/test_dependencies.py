...
analysis used throughout FlowBook (for optimization, validation, etc.).
It identifies:
...
------
Command line:
    flowbook test_dependencies notebook.ipynb -o output.ipynb

Programmatic:
    from flowbook.server.commands.test_dependencies import (
...
from flowbook.util.flowbook_metadata import FlowbookMetadata, DynamicDependencies
...
class TestDependenciesCommand(NotebookCommand):
    ...
    3. Executes all cells sequentially (like ExecuteBaseCommand)
    ...
    async def process(
        ...
        **kwargs,
        ...
        with self.timing_context() as get_elapsed:
            ...
            # Step 3: Enable Scalene (for consistency with ExecuteBaseCommand)
                        ...
                        with timer(key="execute:cell", message=f"Executing cell {idx}:{cell_id}"):
                            ...
                            if source.strip():
                                try:
                                    ...
                                    dynamic_deps = flowbook_meta.get_dynamic_dependencies()
