"""
Agent implementation for FlowBook.
"""
...
from flowbook.agent.llm_cost import cost, full_name
from flowbook.util.output import error
from flowbook.util.text import transform_json


class FlowbookContext:
    """Context for tracking FlowBook agent execution."""
...
@dataclass
class FlowbookStats:
    """Statistics about FlowBook agent execution."""
...
class FlowbookAgent(Agent, Generic[T], RunHooks[FlowbookContext]):
    ...
    async def run(self, input: str) -> Tuple[T, FlowbookStats]:
        context = FlowbookContext()

        key = FlowbookAgent.make_unique(self.name)
        ...
        return result.final_output, FlowbookStats(
            model=self.model, time=context.time, usage=context.usage, log_path=log_path
        )

    async def on_agent_start(
        self, context: RunContextWrapper[FlowbookContext], agent
    ) -> None:
        context.context.start = time.time()

    async def on_agent_end(
        self, context: RunContextWrapper[FlowbookContext], agent, output
    ) -> None:
