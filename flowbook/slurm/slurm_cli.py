#!/usr/bin/env python3
"""Submit FlowBook CLI jobs to Slurm with per-notebook conda environment support.
...
Examples:
    ...
    # Run a different FlowBook command (optimize) without auto-appending notebook
...
Note: The 'flowbook' command with --timings-file and --metadata-file flags
...
def get_flowbook_env_exports() -> str:
    """Generate export statements for all FLOWBOOK_* environment variables.

    Iterates over the current process environment and returns shell export
    statements for any variables starting with 'FLOWBOOK_'.
    """
    exports = []
    for var, value in sorted(os.environ.items()):
        if var.startswith("FLOWBOOK_"):
...
def create_environments(work_items: List[WorkItem], flowbook_source: Path) -> bool:
    ...
    Args:
        ...
        flowbook_source: Path to FlowBook source directory (for pip install -e .)
    ...
    for (env_name, env_dir), notebooks in unique_envs.items():
        ...
        if not setup_environment(env_path, flowbook_source, requirements_file):
...
def setup_environment(
    env_path: Path, flowbook_source: Path, requirements_file: Path
) -> bool:
    """Create/recreate a conda environment with FlowBook and requirements.
    ...
    # 4. Install FlowBook from source -- do after requirements to avoid conflicts
    print(f"[ENV] Installing FlowBook from {flowbook_source}...")
    print(
        f"[ENV] Running: conda run -p {env_path} pip install -v -e {flowbook_source}",
        flush=True,
    )
    env = os.environ.copy()
    env["PYTHONUNBUFFERED"] = "1"
    ...
    if result.returncode != 0:
        print(f"[ERROR] Failed to install FlowBook (exit code: {result.returncode})")
        return False
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Specify the FlowBook subcommand after a literal `--` separator. "
            "The 'flowbook' command with --timings-file and --metadata-file is "
        ...
        epilog=textwrap.dedent(
            ...
            Example commands (after `--`):
              info                   -> runs: flowbook --timings-file=... --metadata-file=... info <notebook>
              execute_all            -> runs: flowbook --timings-file=... --metadata-file=... execute_all <notebook>
              optimize --plan-only   -> runs: flowbook --timings-file=... --metadata-file=... optimize --plan-only <notebook>
    ...
    parser.add_argument(
        "--job-name",
        default="flowbook-batch",
        help="Slurm job name prefix (default: flowbook-batch)",
    )
    parser.add_argument(
        "--env",
        default=None,
...
def build_command_tokens(
    ...
    Automatically prepends the standard flowbook command with timings and metadata flags.
    ...
    """
    # Prepend standard flowbook command with timings and metadata
    cmd_tokens = [
        "flowbook",
...
def run_local_job(work_item: WorkItem, args: argparse.Namespace) -> bool:
    ...
    inner_cmd = textwrap.dedent(
        ...
        # ---- FLOWBOOK environment variables ----
...
def main() -> None:
    ...
    # FlowBook source is the repo root (parent of flowbook/slurm/)
    flowbook_source = Path(__file__).parent.parent.parent

    # Handle --make-env: create/recreate _env environments before submitting jobs
    if args.make_env:
        if not create_environments(work_items, flowbook_source):
