...
at multiple levels: MemoryCheckpoint.diff, handler, and client.

To run these tests:
    pytest flowbook/kernel/test_checkpoint_keys_to_include.py -v
"""
...
from flowbook.kernel_support.kernel_commands import CheckpointCompareRequest
from flowbook.kernel_support.types import MemoryCheckpointDiffResult


class TestCheckpointDiffKeysToInclude:
    """Test MemoryCheckpoint.diff method with keys_to_include parameter."""

    def test_checkpoint_diff_all_keys_when_none(self):
        """Test that MemoryCheckpoint.diff compares all keys when keys_to_include is None."""
        # Create two checkpoints with different values
        cp1 = MemoryCheckpoint("cp1", {'x': 1, 'y': 2, 'z': 3}, {})
        cp2 = MemoryCheckpoint("cp2", {'x': 1, 'y': 999, 'z': 3}, {})

        # Compare without keys_to_include (should compare all)
        result = MemoryCheckpoint.diff(cp1, cp2, keys_to_include=None)
    ...
    def test_checkpoint_diff_specific_keys_only(self):
        """Test that MemoryCheckpoint.diff only compares specified keys."""
        ...
        result = MemoryCheckpoint.diff(cp1, cp2, keys_to_include={'z'})
    ...
    def test_checkpoint_diff_includes_only_different_specified_keys(self):
        ...
        result = MemoryCheckpoint.diff(cp1, cp2, keys_to_include={'x', 'z'})
    ...
    def test_checkpoint_diff_empty_keys_to_include(self):
        ...
        cp1 = MemoryCheckpoint("cp1", {'x': 1, 'y': 2}, {})
        cp2 = MemoryCheckpoint("cp2", {'x': 999, 'y': 999}, {})

        # Compare with empty set
        result = MemoryCheckpoint.diff(cp1, cp2, keys_to_include=set())
    ...
    def test_checkpoint_diff_nonexistent_keys(self):
        ...
        cp1 = MemoryCheckpoint("cp1", {'x': 1, 'y': 2}, {})
        cp2 = MemoryCheckpoint("cp2", {'x': 1, 'y': 2}, {})

        # Compare with keys that don't exist
        result = MemoryCheckpoint.diff(cp1, cp2, keys_to_include={'nonexistent', 'also_missing'})
    ...
    def test_checkpoint_diff_mixed_existing_nonexisting_keys(self):
        ...
        result = MemoryCheckpoint.diff(cp1, cp2, keys_to_include={'x', 'y', 'nonexistent'})
    ...
    def test_checkpoint_diff_complex_types(self):
        ...
        cp1 = MemoryCheckpoint("cp1", {
            'df': pd.DataFrame({'a': [1, 2, 3]}),
            'arr': np.array([1, 2, 3]),
            'num': 42
        }, {})

        cp2 = MemoryCheckpoint("cp2", {
        ...
        result = MemoryCheckpoint.diff(cp1, cp2, keys_to_include={'arr', 'num'})
        ...
        result_all = MemoryCheckpoint.diff(cp1, cp2, keys_to_include=None)
...
class TestHandlerCheckpointCompareKeysToInclude:
    ...
    def test_handle_checkpoint_compare_with_keys_to_include(self, handlers, mock_kernel):
        """Test that handler passes keys_to_include to MemoryCheckpoint.diff."""
        # Setup mocks
        cp1 = MemoryCheckpoint("cp1", {'x': 1, 'y': 2, 'z': 3}, {})
        cp2 = MemoryCheckpoint("cp2", {'x': 999, 'y': 2, 'z': 3}, {})
    ...
    def test_handle_checkpoint_compare_without_keys_to_include(self, handlers, mock_kernel):
        ...
        # Setup mocks
        cp1 = MemoryCheckpoint("cp1", {'x': 1, 'y': 2}, {})
        cp2 = MemoryCheckpoint("cp2", {'x': 999, 'y': 2}, {})
        mock_kernel._checkpoint.get.side_effect = [cp1, cp2]
