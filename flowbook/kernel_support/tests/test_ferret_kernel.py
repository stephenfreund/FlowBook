"""
Tests for kernel/flowbook_kernel.py - ExperimentalKernel class and checkpoint dispatch.
...
from flowbook.kernel_support.experimental_kernel import ExperimentalKernel, CHECKPOINT_COMMANDS
from flowbook.kernel_support.models import ExecutionContext
...
class TestCheckpointCommandsConfig:
    ...
    def test_request_classes_exist(self):
        ...
        from flowbook.kernel_support import kernel_commands as cmd_module
...
class TestExperimentalKernelConfiguration:
    """Tests for ExperimentalKernel configuration constants."""

    def test_default_cell_timeout(self):
        """Default cell timeout is 30 minutes."""
        assert ExperimentalKernel._default_cell_timeout == 30 * 60

    def test_post_kb_grace(self):
        """Post-keyboard-interrupt grace period is 1 second."""
        assert ExperimentalKernel._post_kb_grace == 1.0

    def test_kill_timeout(self):
        """Kill timeout is 3 seconds."""
        assert ExperimentalKernel._kill_timeout == 3.0

    def test_max_passes(self):
        """Max timeout handler passes is 2."""
        assert ExperimentalKernel._max_passes == 2
...
class TestErrorResultFormat:
    ...
    def test_monotonicity_error_format(self):
        ...
        from flowbook.kernel_support.models import MonotonicityViolation
