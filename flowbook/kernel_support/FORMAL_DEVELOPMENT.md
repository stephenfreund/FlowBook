...
monitor that enforces *prefix-consistent read semantics*. We first present a
...
> *Remark.* We do not assume determinism. Cell execution may be
...
*Proof.* By definition, first(t, ℓ) = R(ℓ, v), so t = t₁ · R(ℓ,v) · t₂ where
...
*Proof.* If ℓ ∈ Δ(σ, σ') then σ(ℓ) ≠ σ'(ℓ). By Write Faithfulness,
σ' = apply(t, σ). If no write to ℓ occurs in t, then apply(t, σ)(ℓ) = σ(ℓ),
contradicting σ(ℓ) ≠ σ'(ℓ). ∎

### 1.5 Sequential Reference Execution

Let the notebook consist of cells C₁, …, Cₙ. The *canonical sequential
execution* defines prefix stores:
...
values match *some* sequential execution's prefix stores; determinism would
strengthen this to *the* sequential execution.)
...
*Proof.* No cell has a record; the quantifier is vacuously satisfied. ∎
...
Forward staleness applies only to cells *after* i in notebook order.
...
> *Note.* This check examines all executed cells k > i, regardless of status. A
...
**Definition 1.8.4 (Prefix Checkpoint).** For cell i, the *prefix checkpoint*
is:
...
> *Remark (Recovery from forward contamination).* The path to quiescence after
> forward contamination is to re-execute cells in notebook order. Each step
> uses EXEC-RESTORE: cell i runs from σ^post_{i-1}, producing a clean Σ' that
...
> *Remark (Asymmetry of backward conflict and forward contamination).* Backward
...
*Proof.* Case analysis on the transition rule.
...
- *k < i, fresh:* ¬BackConflict gives Obs_k ∩ Δ(Σ, Σ') = ∅, so
  Σ' =_{Obs_k} Σ. By the pre-transition invariant, Σ =_{Obs_k} σ^pre_k. ✓
- *k > i, fresh:* StaleFwd did not mark k stale, so Obs_k ∩ Δ(Σ, Σ') = ∅.
  Same argument. ✓

**Case EXEC-ACCEPT.** Let C = ⟨Σ, Rec, T⟩ and C' = ⟨Σ', Rec₂, T · t⟩ where
Σ ⇓_{Cᵢ} (Σ', t) and Δ = Δ(Σ, Σ').

We must show: for every k with Rec₂[k] = (σ^pre_k, _, t_k, fresh), we have
Σ' =_{Obs_k} σ^pre_k.

**Sub-case k ≠ i, k < i.** Since ¬BackConflict, for every fresh k < i we have
Obs_k ∩ Δ = ∅, hence Σ' =_{Obs_k} Σ. By the pre-transition invariant,
Σ =_{Obs_k} σ^pre_k. Transitivity gives Σ' =_{Obs_k} σ^pre_k. ✓

**Sub-case k ≠ i, k > i.** If Rec₂[k] is fresh, then StaleFwd did not mark k
stale, so Obs_k ∩ Δ = ∅. The argument proceeds identically. ✓

**Sub-case k = i.** By construction, σ^pre_i = Σ. We must show Σ' =_{Obs_i} Σ.
Partition Obs_i:

- *ℓ ∈ Obs_i \ WS(t):* No write to ℓ in t, so by Write Faithfulness
  Σ'(ℓ) = Σ(ℓ). ✓

- *ℓ ∈ Obs_i ∩ WS(t):* Cell i read ℓ from the pre-store and later wrote a
...
there exists a sequential execution of C₁, …, Cᵢ from σ_init producing prefix
stores σ₀, …, σ_{i-1} such that:
...
*Proof.* By induction on the number of transitions leading to the
configuration.
...
For cell i: σ^pre_i = Σ (the live store at the time of execution). We must
show that there exists a sequential execution of C₁, …, Cᵢ from σ_init whose
store σ_{i-1} agrees with Σ on Obs_i.
...
sequential prefix stores are determined by executing C_j from σ_{j-1}. The
backward conflict check and ¬FwdContaminated together ensure that the values
cell i observes on Obs_i are consistent with a sequential execution of
C₁, …, C_{i-1}.

Formally, construct the sequential prefix stores σ₀, …, σ_{i-1} by executing
C₁, …, C_{i-1} in order from σ_init (choosing any valid execution for each
...
1. *Edited cell:* If Rec[i] ≠ ⊥, set status_i := stale (the recorded trace
   corresponds to old code).

2. *Downstream propagation:* For each fresh cell j > i with
...
3. *All other records unchanged.*

The live store Σ is unchanged: no execution occurs, so no backward conflict
check is needed.

> *Note.* Downstream propagation is conservative: it marks j stale based on the
> *old* write set of cell i. If cell i's new code writes fewer locations, some
...
**Definition 2.5.1.** A configuration ⟨P, ver, Σ, Rec, T⟩ is *quiescent* iff:
...
2. Because every cell is fresh, no cell was recorded via EXEC-CONTAMINATED
   ...
   - EXEC-RESTORE runs cell i from σ^post_{i-1}, so cell i's reads are
...
4. By Theorem 1.10.1, each cell i's observed values match a sequential prefix:
   σ^pre_i =_{Obs_i} σ_{i-1} for some sequential execution of P_f(1), …,
   P_f(i).
