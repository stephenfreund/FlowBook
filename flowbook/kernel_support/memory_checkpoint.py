"""
MemoryCheckpoint management for kernel state snapshots.
...
Key Components:
  ...
  MemoryCheckpoint          - Single snapshot containing deep-copied namespace
  MemoryCheckpoints         - Manager for multiple named checkpoints
...
Data Flow:
  user_ns → filter → deep copy → MemoryCheckpoint
  MemoryCheckpoint → deep copy → user_ns (on restore)
...
---------------------
Optional sanity_check=True mode in MemoryCheckpoints:
...
7.3 Namespace Changes During MemoryCheckpoint
---------------------------------------
...
Cache management:
  - Cache cleared when all checkpoints are deleted (MemoryCheckpoints.clear())
  - Cache cleared when last checkpoint is deleted (MemoryCheckpoints.delete())
...
Example 1: Basic Save/Restore
    from flowbook.kernel_support.checkpoint import MemoryCheckpoints
...
Example 6: Comparing MemoryCheckpoints
    from flowbook.kernel_support.checkpoint import MemoryCheckpoint
    ...
    diff_result = MemoryCheckpoint.diff(cp1, cp2)
    if diff_result.differences:
        print("Variables changed:")
        for diff in diff_result.differences:
            print(f"  {diff}")

Example 7: Managing Multiple MemoryCheckpoints
    cp = MemoryCheckpoints()
...
Example 8: Performance Tips
    ...
    print(f"MemoryCheckpoint took {time.time() - start:.2f} seconds")
...
------------------------
MemoryCheckpoints support deep alias detection - identifying when variables share
...
~~~~~~~~~~~~~~~~~~~~
The alias index is built LAZILY on first query and stored in MemoryCheckpoint:
...
---------------------------
MemoryCheckpoint.diff() includes timing for debugging performance issues:
...
-----------------------
  ...
  - [ ] MemoryCheckpoint compression for memory efficiency
  - [ ] Async checkpoint creation (background deep copy)
  - [ ] MemoryCheckpoint serialization (save to disk)
...
supported throughout the checkpoint system. This includes:
  ...
  - MemoryCheckpoint diffing between MultiIndex DataFrames
...
class MemoryCheckpoints:
    ...
    def _deep_copy_user_ns(
        ...
        for k, v in variables.items():
            try:
                # Measure object size only when profiling (expensive for complex objects)
