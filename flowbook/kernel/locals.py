...
from flowbook.kernel.extended_types import get_type_model
...
def _format_limited(
    ...
    def format_object(obj, depth):
        ...
        def valid_field(attr):
            value = getattr(obj, attr, None)
            return (value is None or not callable(value)) and not attr.startswith("__")

        attributes = dir(obj)
        fields = {
            attr: getattr(obj, attr, None) for attr in attributes if valid_field(attr)
        }
        return format(
...
def print_locals(file: StringIO, frame: FrameType) -> None:
    ...
    if len(defined_locals) > 0:
        ...
        for name in sorted(defined_locals):
            ...
            t = get_type_model(value)
            prefix = f"      {name}: {t} = "
            rep_list = _format_limited(value, limit=10).split("\n")
            if len(rep_list) > 1:
                rep = (
                    prefix
                    + rep_list[0]
