#!/usr/bin/env python3
"""Launch FlowBook kernel with Scalene memory tracking enabled.

This launcher runs the FlowBook kernel under Scalene's memory profiler,
which properly initializes Scalene and enables accurate memory tracking.

Usage:
    python -m flowbook.kernel.launch_with_scalene -f connection_file
...
3. Once under Scalene, starts the FlowBook kernel normally

Environment Variables (set by Scalene):
    macOS: DYLD_INSERT_LIBRARIES=/path/to/libscalene.dylib
    Linux: LD_PRELOAD=libscalene.so, LD_LIBRARY_PATH updated
"""
...
def launch_kernel():
    """Launch the FlowBook kernel (called when running under Scalene)."""
...
def main(argv: Optional[list] = None):
    ...
    cmd = [
        ...
        "-m", "flowbook.kernel.launch_with_scalene",
    ]
