...
class TestAliasIndexOptimizations:
    ...
    def test_path_tracking_disabled_by_default(self):
        """Verify path tracking is disabled when FLOWBOOK_LOG_DEEP_ALIASES is not set."""
        import os

        # Ensure env var is not set
        old_val = os.environ.pop("FLOWBOOK_LOG_DEEP_ALIASES", None)
        try:
            shared = {"value": 42}
            namespace = {"a": {"ref": shared}, "b": {"ref": shared}}
            ...
            if old_val is not None:
                os.environ["FLOWBOOK_LOG_DEEP_ALIASES"] = old_val
