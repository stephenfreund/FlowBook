...
from flowbook.lsp.session import LspSession
from flowbook.lsp.utils import (
    as_uri,
    cell_uri_to_code,
    cell_uri_to_index,
...
from flowbook.util.dependencies import CellDependencies, analyze_notebook
from flowbook.util.flowbook_metadata import ProfileData, OptimizationPotential, FlowbookMetadata
from flowbook.util.output import log, timer, error
import textwrap
    ...
    def tools(self, include_profile: bool = False) -> list[Tool]:
        ...
        def get_profile(cell_id: str) -> ProfileData:
            ...
            with timer(key="tool:get_profile", message=f"get_profile({cell_id})"):
                ...
                if cell is None:
                    raise ValueError(f"Cell {cell_id} not found")
                data = FlowbookMetadata.from_cell(cell)
