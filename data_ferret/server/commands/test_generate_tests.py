...
from data_ferret.server.commands.generate_tests import (
    GenerateTestsCommand,
    GeneratedTests,
)
from data_ferret.util.ferret_metadata import UnitTest, UnitTests, FerretMetadata
...
class TestGenerateTestsCommand:
    ...
    def create_notebook_with_existing_tests(self):
        ...
        cell.metadata["ferret"] = {"unit_tests": unit_tests.model_dump()}
    ...
    async def test_generate_tests_adds_to_existing(self):
        ...
        with patch('data_ferret.server.commands.generate_tests.FerretAgent.make_and_run_agent') as mock_agent:
            ...
            assert ferret_meta.unit_tests is not None
            assert len(ferret_meta.unit_tests.tests) == 3  # 1 existing + 2 new

            # Check that existing test is still there
            test_titles = [t.title for t in ferret_meta.unit_tests.tests]
    ...
    async def test_generate_tests_with_dependencies(self):
        ...
        with patch('data_ferret.server.commands.generate_tests.FerretAgent.make_and_run_agent') as mock_agent:
            ...
            # The input should contain globals_read and globals_written info
            input_text = call_args[1]["input"]
            assert "Variables this cell reads" in input_text
            assert "Variables this cell writes" in input_text
...
if __name__ == "__main__":
    ...
    print("Run 'pytest data_ferret/server/commands/test_generate_tests.py' for async tests")
