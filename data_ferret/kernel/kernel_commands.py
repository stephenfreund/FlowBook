...
class CheckpointSaveResponse(KernelCommandResponse):
    ...
    saved: Dict[str, TypeModel] = Field(
        ...
        description="Variables that were successfully saved"
    )
    removed: Dict[str, TypeModel] = Field(
        default_factory=dict,
        description="Variables that could not be saved"
    )
...
class CheckpointListResponse(KernelCommandResponse):
    ...
    checkpoints: List[str] = Field(
        ...
        description="Names of all available checkpoints"
    )
...
class FinalMessage(BaseModel):
    ...
    response: Optional[Dict[str, Any]] = Field(
        None,
        description="Response data (present if ok=True)"
    )
    error: Optional[str] = Field(
        None,
        description="Error message (present if ok=False)"
    )
