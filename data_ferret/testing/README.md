...
```bash
# Basic usage
python -m data_ferret.testing.scripts.run_correctness <notebook.ipynb>
...
```bash
# Basic usage
python -m data_ferret.testing.scripts.run_performance <notebook.ipynb>
...
python -m data_ferret.testing.scripts.run_correctness data_ferret/testing/notebooks/multi_dataframe.ipynb

# Thorough performance test with verbose output
python -m data_ferret.testing.scripts.run_performance data_ferret/testing/notebooks/ml_workflow.ipynb -n 50 -m 5 -s 123 -v

# Test your own notebook
python -m data_ferret.testing.scripts.run_correctness examples/Example.ipynb -n 10 -s 42
```

## Test Notebooks

Located in `data_ferret/testing/notebooks/`:
...
- `data_ferret.kernel.checkpoint.Checkpoints` - State snapshots
- `data_ferret.kernel.checkpoint.Checkpoint.diff()` - State comparison
- `data_ferret.kernel.tracking.TrackingDict` - Read/write tracking
- `data_ferret.sdc_kernel.sdc_enforcer.SDCEnforcer` - SDC enforcement

### Isolation Strategy

The framework is completely isolated:

- All code in separate `data_ferret/testing/` directory
...
4. Run correctness test to verify: `python -m data_ferret.testing.scripts.run_correctness notebooks/your_notebook.ipynb`

### Programmatic Usage

```python
from data_ferret.testing import (
    load_notebook,
    SDCSimulator,
