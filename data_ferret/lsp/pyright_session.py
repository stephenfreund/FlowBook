...
# from data_ferret.lsp.utils import as_uri
# from data_ferret.lsp.defaults import default_initialize
...
# class PyrightLspSession(MethodDispatcher):
#     """Send and Receive messages over LSP as a test LS Client (Pyright)."""
...
#     # --- helper to locate pyright-langserver ---------------------------------
#     def _resolve_pyright_cmd(self):
#         """
#         Find the pyright-langserver executable. Resolution order:
#         1. PYRIGHT_LANGSERVER env var (absolute or on PATH)
#         2. 'pyright-langserver' (POSIX) / 'pyright-langserver.cmd' (Windows) on PATH
#         3. local node_modules/.bin under cwd (useful when installed locally)
#         """
#         # 1) explicit override
#         env_cmd = os.environ.get("PYRIGHT_LANGSERVER")
#         if env_cmd:
#             return env_cmd
...
#         local_bin = os.path.join(self.cwd, "node_modules", ".bin")
#         for c in candidates:
#             p = os.path.join(local_bin, c)
#             if os.path.isfile(p):
#                 return p

#         raise FileNotFoundError(
#             "Unable to find 'pyright-langserver'. Install with `npm i -g pyright` "
#             "or set PYRIGHT_LANGSERVER to its path."
#         )

#     # add these methods to LspSession
#     def _client_register_capability(self, params):
#         # params: { "registrations": [{ "id": str, "method": str, "registerOptions": any }...] }
#         for reg in params.get("registrations", []):
#             self._registered_capabilities[reg["id"]] = reg
#         return None  # returning successfully acknowledges registration

#     def _client_unregister_capability(self, params):
#         # params: { "unregisterations": [{ "id": str, "method": str }...] }
#         for unreg in params.get("unregisterations", []):
#             self._registered_capabilities.pop(unreg["id"], None)
#         return None

#     def _workspace_configuration(self, params):
#         """
#         Pyright calls workspace/configuration to read settings.
#         Return a list of settings objects (one per item in params['items']).
#         Provide empty dicts or your desired pyright config.
#         """
#         items = params.get("items", [])
#         # Example: return empty settings for all
#         return [{} for _ in items]

#     def __enter__(self):
#         """Context manager entrypoint."""
#         server_cmd = [self._resolve_pyright_cmd(), "--stdio"]

#         # Launch pyright-langserver over stdio
#         # Note: shell=False because we're not spawning a Python module now.
...
#             "client/registerCapability": self._client_register_capability,
#             "client/unregisterCapability": self._client_unregister_capability,
#             "workspace/configuration": self._workspace_configuration,
#         }
...
#     #             else default_initialize(self.root)
#     #         ),
...
#     # add these methods to LspSession
#     def _log(self, *a):
#         print("[LSP-CLIENT]", *a, file=sys.stderr, flush=True)
...
#             rid = reg.get("id")
#             meth = reg.get("method")
#             self._registered_capabilities[rid] = reg
#             self._log(
#                 f"dynamic registration: id={rid} method={meth} options={json.dumps(reg.get('registerOptions', {}))}"
#             )
#         return None
...
#         for unreg in params.get("unregisterations", []):
#             rid = unreg.get("id")
#             meth = unreg.get("method")
#             self._registered_capabilities.pop(rid, None)
#             self._log(f"dynamic unregistration: id={rid} method={meth}")
#         return None

#     def _workspace_configuration(self, params):
#         # Return empty settings objects (one per item). Adjust if you want custom pyright settings.
#         items = params.get("items", [])
#         return [{} for _ in items]
...
#         process_server_capabilities=None,
#     ):
#         """Sends the initialize request to LSP server and logs definition support."""
#         server_initialized = Event()

#         def _after_initialize(fut):
#             init_result = fut.result() or {}
#             caps = init_result.get("capabilities", {})
#             self._log("initialize.result capabilities =", json.dumps(caps, indent=2))

#             # Check for definition provider presence
#             defprov = caps.get("definitionProvider")
#             if isinstance(defprov, bool):
#                 self._log(f"definitionProvider (bool) = {defprov}")
#             elif isinstance(defprov, dict):
#                 # DefinitionOptions / DefinitionRegistrationOptions
#                 self._log("definitionProvider (options) =", json.dumps(defprov))
#             else:
#                 self._log(
#                     "definitionProvider not advertised in initialize result (may be registered dynamically)."
#                 )

#             if process_server_capabilities:
#                 process_server_capabilities(init_result)
...
#         # optional: short wait-loop to surface if nothing arrives
#         if not server_initialized.wait(timeout=10):
#             self._log("WARNING: initialize response not received within 10s")
...
#         self._send_notification("notebookDocument/didChange", params=did_change_params)

#     def notify_did_save_notebook_document(self, did_save_params):
#         """Sends did save notebook document notification to LSP Server."""
#         self._send_notification("notebookDocument/didSave", params=did_save_params)

#     def notify_did_open_notebook_document(self, did_open_params):
#         """Sends did open notebook document notification to LSP Server."""
#         self._send_notification("notebookDocument/didOpen", params=did_open_params)

#     def notify_did_close_notebook_document(self, did_close_params):
#         """Sends did close notebook document notification to LSP Server."""
#         self._send_notification("notebookDocument/didClose", params=did_close_params)

#     def open_notebook_document(self, path, notebook: NotebookNode | None = None):
...
#     # def _send_request(self, method, params=None, handle_response=None, timeout=None):
#     #     """
#     #     Send an LSP request.
#     #     - handle_response: callable(result) or None
#     #     - timeout: if set, wait synchronously and return the result (or raise)
#     #     """
#     #     params = {} if params is None else params
#     #     fut = self._endpoint.request(method, params)
...
#     #                 self._log(
#     #                     f"REQUEST ERROR method={method} params={json.dumps(params)} err={e}"
#     #                 )
#     #                 return
#     #             except Exception as e:
#     #                 self._log(f"REQUEST EXC method={method} err={e!r}")
#     #                 return
#     #             try:
#     #                 handle_response(res)  # pass the resolved result, not the Future
#     #             except Exception as e:
#     #                 self._log(f"handler failed for {method}: {e!r}")

#     #         fut.add_done_callback(_cb)

#     #     if timeout is not None:
#     #         # synchronous wait path (useful in tests)
#     #         try:
#     #             res = fut.result(timeout=timeout)
#     #             return res
#     #         except JsonRpcException as e:
#     #             self._log(f"REQUEST TIMEOUT/ERROR method={method} err={e}")
#     #             raise
#     #     return fut
