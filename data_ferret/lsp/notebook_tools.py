...
from data_ferret.ferret_data import get_ferret_data, OptimizationStep
...
from data_ferret.profiler.user_ns import CellNS
...
class ProfileData(BaseModel):
    duration: float | None = Field(title="The duration of the cell in seconds")
    profile: str | None = Field(title="The Scalene profile report for the cell")


class OptimizationPotential(BaseModel):
    optimization_potential: int = Field(
        description="How optimizable is this cell? (0-5)"
    )
    explanation: str = Field(
        description="The highest priority concrete suggestion for how to optimize this cell"
    )
    steps: List[OptimizationStep] = Field(description="The steps to optimize the cell.")
...
class NotebookTools(LspSession):
    ...
    def tools(self, include_profile: bool = False) -> list[Tool]:
        ...
        @function_tool
        def get_cell_ids() -> list[str]:
            """
            Returns the ids of the cells in the notebook, in order.
            """
            result = [cell["id"] for cell in self.notebook["cells"]]
            log(f"get_cell_ids() -> ...")
            return result
        ...
        def set_source(cell_id: str, source: str) -> CellContents:
            ...
            if result is None:
                raise ValueError(f"Cell {cell_id} not found")
            log(f"set_source({cell_id}, {repr(source)[:20]}...)")
            return result
        ...
        def get_cells_for_definition(cell_id: str, symbol: str) -> List[CellContents]:
            ...
            log(
                f"get_cells_for_definition({cell_id}, {symbol}) -> {[c.id for c in result]}"
            )
            return result
        ...
        def get_profile(cell_id: str) -> ProfileData:
            ...
            # with timer(key="tool_get_profile", message=f"Getting profile for {cell_id}"):
            cell = self.get_cell_by_id(cell_id)
            if cell is None:
                raise ValueError(f"Cell {cell_id} not found")
            data = get_ferret_data(cell)
            if data.profile is None:
                raise ValueError(f"Profile for {cell_id} not found")
            log(f"get_profile({cell_id}) -> ...")
            return ProfileData(
                duration=data.profile.duration, profile=data.profile.profile
            )
        ...
        def get_optimization_potential(cell_id: str) -> OptimizationPotential:
            ...
            """
            # with timer(key="tool_get_optimization_potential", message=f"Getting optimization potential for {cell_id}"):
            cell = self.get_cell_by_id(cell_id)
            if cell is None:
                raise ValueError(f"Cell {cell_id} not found")
            data = get_ferret_data(cell)
            if data.inspection is None:
                raise ValueError(f"Inspection for {cell_id} not found")
            log(
                f"get_optimization_potential({cell_id}) -> {data.inspection.optimization_potential} ..."
            )
            return OptimizationPotential(
                optimization_potential=data.inspection.optimization_potential,
                explanation=data.inspection.explanation,
                steps=data.inspection.steps,
            )

        @function_tool
        def get_input_variables(cell_id: str) -> Dict[str, str]:
            """Get the input variables and their types for a cell."""
            # with timer(key="tool_get_input_variables", message=f"Getting input variables for {cell_id}"):
            cell = self.get_cell_by_id(cell_id)
            if cell is None:
                raise ValueError(f"Cell {cell_id} not found")
            data = get_ferret_data(cell)
            if data.profile is None:
                raise ValueError(f"Input variables for {cell_id} not found")
            if data.profile.cell_ns is None:
                raise ValueError(f"Input variables for {cell_id} not found")
            user_ns = data.profile.cell_ns.in_vars
            if user_ns is None:
                raise ValueError(f"Input variables for {cell_id} not found")
            log(f"get_input_variables({cell_id}) -> ...")
            return {
                var: f"{item.value.model_type}" for var, item in user_ns.vars.items()
            }
        ...
        if include_profile:
            tools.extend([get_profile, get_optimization_potential, get_input_variables])
        return tools
    ...
    def get_source(self, cell_id: str) -> CellContents | None:
        ...
        if cell is None:
            return None
        return CellContents(id=cell["id"], code=cell["source"])
