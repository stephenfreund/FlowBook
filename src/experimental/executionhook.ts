 ...
 * and extract flowbook metadata from kernel outputs
 */
...
export class ExecutionHookManager {
  ...
  private setupHooks(): void {
    ...
    // Listen for cell execution completion to extract flowbook metadata
  ...
  private extractFlowbookMetadata(
    ...
    for (const output of outputs) {
      ...
      const flowbookMeta: Partial<IFlowbookMetadata> = {};

      // Extract profile metadata
      if (metadata.profile) {
        flowbookMeta.profile = metadata.profile as IFlowbookProfileData;
      }
      ...
      if (trackingData) {
        flowbookMeta.dynamic_dependencies = {
      ...
      if (Object.keys(flowbookMeta).length > 0) {
        return flowbookMeta;
      }
    }
    return null;
  ...
  /**
   * Handle cell execution completion to extract flowbook metadata from outputs
   */
  private onCellExecuted(
    ...
    // Extract flowbook metadata from outputs
    const flowbookMeta = this.extractFlowbookMetadata(outputs);
    if (!flowbookMeta) {
      return;
    }
    ...
    cell.model.setMetadata('flowbook', mergedMeta);

    console.log(
      `ExecutionHook: Extracted flowbook metadata for cell ${cell.model.id}:`,
      flowbookMeta
    );
  }
}
