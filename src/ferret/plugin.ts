/**
 * Ferret Kernel Plugin - Activates only for ferret_kernel
 */
...
class FerretActivationManager {
  ...
  constructor(
    ...
    this._manager = new FerretCommandsManager(app, tracker, this._historyManager);
  ...
  private _setupKernelChangeListener(): void {
    this._kernelDetector.kernelChanged.connect((_, info) => {
      if (info.currentKernel === 'ferret_kernel') {
        this._activateUI();
      } else if (info.previousKernel === 'ferret_kernel') {
        this._deactivateUI();
      }
    });
  ...
  private _checkCurrentNotebook(): void {
    ...
    if (notebook) {
      notebook.sessionContext.ready.then(() => {
        if (this._kernelDetector.isFerretKernel(notebook)) {
          this._activateUI();
        } else {
          this._deactivateUI();
        }
  ...
  private _activateUI(): void {
    ...
    console.log('Ferret Plugin: Activating UI for ferret_kernel');
    ...
    this._metadataPanel = new FerretMetadataPanel();
    ...
    console.log('Ferret Plugin: UI activated');
  }
  ...
  private _deactivateUI(): void {
    ...
    console.log('Ferret Plugin: Deactivating UI');
    ...
    console.log('Ferret Plugin: UI deactivated');
  }
}

/**
 * Ferret Plugin definition
 */
export const ferretPlugin: JupyterFrontEndPlugin<void> = {
  ...
  activate: (
    ...
    console.log('Ferret Plugin: Extension registered (will activate UI when ferret_kernel is used)');
    new FerretActivationManager(app, tracker, palette, toolbarRegistry);
  }
};
